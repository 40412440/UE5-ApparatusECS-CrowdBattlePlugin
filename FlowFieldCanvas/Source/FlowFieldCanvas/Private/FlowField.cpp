...
// 在 AFlowField.cpp 中实现 A* 算法
bool AFlowField::FindPathAStar(const FVector2D& StartCoord, const FVector2D& GoalCoord, TArray<FVector>& OutPath)
{
	// 验证坐标有效性
	auto IsValidCoord = [&](FVector2D Coord) -> bool {
		return Coord.X >= 0 && Coord.X < xNum && Coord.Y >= 0 && Coord.Y < yNum;
		};
	...
	// 检查起点终点是否为障碍
	if (CurrentCellsArray[StartIndex].cost == 255 || CurrentCellsArray[GoalIndex].cost == 255) {
		return false;
	}
	...
	gScore.Init(FLT_MAX, CurrentCellsArray.Num());
	CameFrom.Init(FVector2D(-1, -1), CurrentCellsArray.Num());
	...
	while (!OpenSet.empty()) {
		...
		OpenSet.pop();
		const int32 CurrentIndex = CoordToIndex(CurrentCoord);
		...
		if (CurrentCoord == GoalCoord) {
			...
			while (TraceCoord != StartCoord) {
				...
				TraceCoord = CameFrom[CoordToIndex(TraceCoord)];
			}
			...
			for (const FVector2D& Coord : PathCoords) {
				OutPath.Add(CurrentCellsArray[CoordToIndex(Coord)].worldLoc);
			}
			return true;
		}
		...
		for (int32 i = 0; i < NeighborCoords.Num(); ++i) {
			...
			FCellStruct& NeighborCell = CurrentCellsArray[NeighborIndex];
			FCellStruct& CurrentCell = CurrentCellsArray[CurrentIndex];
			...
			auto IsValidDiagonal = [&](TArray<FVector2D> neighborCoords, int32 indexA, int32 indexB) -> bool
				{
					...
					if (IsValidCoord(neighborCoords[indexA]))
					{
						if (CurrentCellsArray[CoordToIndex(neighborCoords[indexA])].cost == 255)
						{
							result = false;
						}
					}
					if (IsValidCoord(neighborCoords[indexB]))
					{
						if (CurrentCellsArray[CoordToIndex(neighborCoords[indexB])].cost == 255)
						{
							result = false;
						}
					}
			...
			// 对角线移动检查
			if (Style == EStyle::AdjacentFirst && i < 4) { // 前4个是斜角
			...
			const float HorizontalDist = FVector2D::Distance(
			...
			// 计算移动成本
			const float MoveCost = (NeighborCell.cost == 255) ? FLT_MAX : static_cast<float>(NeighborCell.cost);
...
FVector AFlowField::GetSteeringDirection(const FVector& CurrentLocation, const TArray<FVector>& PathPoints, float MoveSpeed, float LookAheadDistance, float PathRadius)
{
	// 1. 验证路径有效性
	if (PathPoints.Num() < 2)
	{
		// 路径点不足，返回零向量或最后一点的方向
		if (PathPoints.Num() == 1) return (PathPoints[0] - CurrentLocation).GetSafeNormal();
	...
	// 3. 计算预测目标点（考虑移动速度和视线距离）
	FVector TargetPoint = ClosestPointOnPath;

	// 计算路径方向
	FVector PathDirection = (PathPoints[ClosestSegmentIndex + 1] - PathPoints[ClosestSegmentIndex]).GetSafeNormal();

	// 计算预测距离（速度越大看得越远）
	float PredictDistance = FMath::Max(LookAheadDistance, MoveSpeed * 0.5f);

	// 沿着路径向前移动预测距离
	TargetPoint += PathDirection * PredictDistance;

	// 4. 确保目标点在路径上（不超过下一个路径点）
	float SegmentLength = FVector::Dist(PathPoints[ClosestSegmentIndex], PathPoints[ClosestSegmentIndex + 1]);
	float DistanceAlongSegment = FVector::Dist(PathPoints[ClosestSegmentIndex], TargetPoint);

	if (DistanceAlongSegment > SegmentLength && ClosestSegmentIndex < PathPoints.Num() - 2)
	{
		// 超出当前线段，移动到下一段
		float Remainder = DistanceAlongSegment - SegmentLength;
		TargetPoint = PathPoints[ClosestSegmentIndex + 1] + (PathPoints[ClosestSegmentIndex + 2] - PathPoints[ClosestSegmentIndex + 1]).GetSafeNormal() * Remainder;
	}

	// 5. 计算并返回方向向量
	FVector Direction = (TargetPoint - CurrentLocation);
	Direction.Z = 0; // 忽略垂直方向（2D平面）

	// 归一化处理
	if (Direction.SizeSquared() > KINDA_SMALL_NUMBER) return Direction.GetSafeNormal();

	return FVector::ZeroVector;
}

// 辅助函数：计算点到线段的最短距离和最近点
FVector AFlowField::FindClosestPointOnSegment(const FVector& Point, const FVector& SegmentStart, const FVector& SegmentEnd)
{
	...
	// 计算投影比例
	const float T = FVector::DotProduct(Point - SegmentStart, Segment) / SegmentLengthSquared;
